...
class Track:
    ...
    def sort_key(self) -> str:
        return f'{self.playlist_name}-{self.artist}-{self.name}-{self.album_name}-{self.disc_number:03d}-{self.track_number:03d}'.lower()

    @property
    def compare_key(self) -> str:
        return f'{self.artist.lower()}-{self.name.lower()}'.lower()
...
class API:
    ...
    def get_playlists(self) -> list[dict]:
        ...
        while True:
            ...
            response = self.api.user_playlists(user=self.username, offset=offset, limit=3)
            ...
            playlists.extend([playlist for playlist in items if playlist['owner']['id'] == self.username])
